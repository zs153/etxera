...
const buildTable = (state) => {
  ...
  myList.map(element => {
    ...
    if (element.STAUSU === estadosEnvio.enviado) {
    ...
    cell.innerHTML = `<div class="d-flex align-items-center">
      <div class="flex-fill">
        <div class="font-weight-medium">${element.FECENV}</div>
      </div>
    </div>`
    row.appendChild(cell)
    ...
    cell.innerHTML = `<ul class="dots-menu">
      <li class="nav-item drop-right">
        ...
        <ul>
          <li class="nav-item">
            <a href="/admin/envios/supender/${element.IDENVI}" class="nav-link">
              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-inline me-2" width="24" height="24" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
          ...
          <li class="nav-item">
            <a href="#" class="nav-link" onclick="{document.getElementById('idenvi').value ='${element.IDENVI}', document.getElementById('msgbor').innerHTML ='<p>${element.FECENV}</p><p>${element.URLENV}</p>'}" data-bs-toggle="modal" data-bs-target="#modal-borrar">
              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-inline me-2" width="24" height="24" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
...
const createPages = () => {
  ...
  if (hasPrevUsers) {
    str += "<li class='page-item previous no'><a href='/user?cursor=" + JSON.stringify(cursor) + "&part=" + document.getElementById('buscarEnvioBox').value + "&dir=prev' class='nav-link'>&#9664 Anterior</a>";
  }

  if (hasNextUsers) {
    str += "<li class='page-item next no'><a href='/user?cursor=" + JSON.stringify(cursor) + "&part=" + document.getElementById('buscarEnvioBox').value + "&dir=next' class='nav-link'>Siguiente &#9654</a>";
  }
  str += "</ul>";
